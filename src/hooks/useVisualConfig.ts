...
function setNonNegativeIntFromStringInDoc(doc: YamlDocument, path: YamlPath, value: unknown): void {
  const safe = typeof value === 'string' ? value : '';
  const trimmed = safe.trim();
  if (trimmed === '') {
    if (docHas(doc, path)) doc.deleteIn(path);
...
export function parseVisualConfigValuesFromYaml(yamlContent: string): VisualConfigValues {
  ...
  return {
    ...
    quotaSwitchProject: Boolean(quotaExceeded?.['switch-project'] ?? DEFAULT_VISUAL_VALUES.quotaSwitchProject),
    quotaSwitchPreviewModel: Boolean(quotaExceeded?.['switch-preview-model'] ?? DEFAULT_VISUAL_VALUES.quotaSwitchPreviewModel),
    ...
    routingCodexPlusPlanPriority: routingCodexPlanPriorityValue(routingCodexPlanPriorities, 'plus'),
    routingCodexTeamPlanPriority: routingCodexPlanPriorityValue(routingCodexPlanPriorities, 'team'),
...
export function applyVisualConfigValuesToYaml(
  ...
  values: VisualConfigValues,
  dirtyFields: Set<string>,
  baselineValues: VisualConfigValues = parseVisualConfigValuesFromYaml(currentYaml)
): string {
  try {
    ...
    const quotaAutoDisableDirtyFields = [
      'quotaAutoDisableEnabled',
      'quotaAutoDisableAutoEnable',
      'quotaAutoDisableScanIntervalSeconds',
      'quotaAutoDisableScanConcurrency',
    ...
    const quotaAutoDisableDirty = quotaAutoDisableDirtyFields.some((field) =>
      dirtyFields.has(field)
    );
    if (quotaAutoDisableDirty) {
      ensureMapInDoc(doc, ['quota-auto-disable']);

      if (dirtyFields.has('quotaAutoDisableEnabled')) {
        doc.setIn(['quota-auto-disable', 'enabled'], values.quotaAutoDisableEnabled);
      }
      if (dirtyFields.has('quotaAutoDisableAutoEnable')) {
        doc.setIn(['quota-auto-disable', 'auto-enable'], values.quotaAutoDisableAutoEnable);
      }
      if (dirtyFields.has('quotaAutoDisableScanIntervalSeconds')) {
        setIntFromStringInDoc(
          doc,
      ...
      if (dirtyFields.has('quotaAutoDisableScanConcurrency')) {
        setIntFromStringInDoc(
          doc,
      ...
      if (dirtyFields.has('quotaAutoDisableScanRateLimitPerSecond')) {
        setIntFromStringInDoc(
          doc,
      ...
      if (dirtyFields.has('quotaAutoDisableProbeTimeoutSeconds')) {
        setIntFromStringInDoc(
          doc,
      ...
      if (dirtyFields.has('quotaAutoDisableMinCapacityCoveragePercent')) {
      ...
      [
        'pro-only',
        'threshold-percent',
        'five-hour-threshold-percent',
        'weekly-threshold-percent',
        'resume-five-hour-threshold-percent',
        'resume-weekly-threshold-percent',
        'interval-seconds',
        'max-scan-per-run',
        'auto-enable-scan-reserve',
        'sample-freshness-seconds',
        'account-error-backoff-seconds',
        'transient-error-backoff-seconds',
      ].forEach((key) => doc.deleteIn(['quota-auto-disable', key]));

      const writePlanPolicy = (
        plan: 'pro' | 'plus' | 'team',
        enabledField: string,
        ...
        const policyDirty =
          dirtyFields.has(enabledField) ||
          dirtyFields.has(thresholdField) ||
          dirtyFields.has(resumeField);
        if (!policyDirty) return;

        const policyPath = ['quota-auto-disable', 'plan-policies', plan];
        ensureMapInDoc(doc, policyPath);
        if (dirtyFields.has(enabledField)) doc.setIn([...policyPath, 'enabled'], enabled);
        if (dirtyFields.has(thresholdField)) {
          setIntFromStringInDoc(doc, [...policyPath, 'threshold-percent'], threshold);
        }
        if (dirtyFields.has(resumeField)) {
          setIntFromStringInDoc(doc, [...policyPath, 'resume-threshold-percent'], resumeThreshold);
        }
        if (requireWindows) {
          doc.setIn([...policyPath, 'require-five-hour-window'], true);
          doc.setIn([...policyPath, 'require-weekly-window'], true);
        }
        deleteIfMapEmpty(doc, policyPath);
      };
      ...
      deleteIfMapEmpty(doc, ['quota-auto-disable', 'plan-policies']);

      const capacityAlertsDirty =
      ...
      if (capacityAlertsDirty) {
        const alertsPath = ['quota-auto-disable', 'capacity-alerts'];
        ensureMapInDoc(doc, alertsPath);
        if (dirtyFields.has('quotaCapacityAlertsEnabled')) {
          doc.setIn([...alertsPath, 'enabled'], values.quotaCapacityAlertsEnabled);
        }
        if (dirtyFields.has('quotaCapacitySnapshotsIncluded')) {
          doc.setIn([...alertsPath, 'include-snapshots'], values.quotaCapacitySnapshotsIncluded);
        }

        const writeCapacityPlan = (
          plan: 'pro' | 'plus' | 'team',
          fiveHourField: string,
          weeklyField: string,
          fiveHourValue: string,
          weeklyValue: string
        ) => {
          if (!dirtyFields.has(fiveHourField) && !dirtyFields.has(weeklyField)) return;
          const planPath = [...alertsPath, 'plans', plan];
          ensureMapInDoc(doc, planPath);
          if (dirtyFields.has(fiveHourField)) {
            setNumberFromStringInDoc(
              doc,
              [...planPath, 'five-hour-threshold-equivalent'],
              fiveHourValue
            );
          }
          if (dirtyFields.has(weeklyField)) {
            setNumberFromStringInDoc(
              doc,
              [...planPath, 'weekly-threshold-equivalent'],
              weeklyValue
            );
          }
          deleteIfMapEmpty(doc, planPath);
        };
        ...
        deleteIfMapEmpty(doc, [...alertsPath, 'plans']);
        deleteIfMapEmpty(doc, alertsPath);
      }

      if (dirtyFields.has('quotaAutoDisableProFiveHourCapacityAlertThreshold')) {
        setNumberFromStringInDoc(
          doc,
    ...
    if (routingDirty) {
      ...
      const codexPlanPrioritiesDirty =
        dirtyFields.has('routingCodexProPlanPriority') ||
        dirtyFields.has('routingCodexPlusPlanPriority') ||
        dirtyFields.has('routingCodexTeamPlanPriority');
      if (codexPlanPrioritiesDirty) {
        ensureMapInDoc(doc, ['routing', 'codex-plan-priorities']);
        if (dirtyFields.has('routingCodexProPlanPriority')) {
        ...
        if (dirtyFields.has('routingCodexPlusPlanPriority')) {
          setNonNegativeIntFromStringInDoc(
            doc,
        ...
        if (dirtyFields.has('routingCodexTeamPlanPriority')) {
          setNonNegativeIntFromStringInDoc(
            doc,
...
export function useVisualConfig() {
  ...
  const applyVisualChangesToYaml = useCallback(
    (currentYaml: string): string =>
      applyVisualConfigValuesToYaml(currentYaml, visualValues, dirtyFields, baselineValues),
