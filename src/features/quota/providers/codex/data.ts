...
export const buildCodexQuotaDataFromUsageBody = (
  file: AuthFileItem,
  body: unknown,
  t: TFunction
): CodexQuotaData => {
...
const fetchCodexQuota = async (file: AuthFileItem, t: TFunction): Promise<CodexQuotaData> => {
  ...
  const data = buildCodexQuotaDataFromUsageBody(file, result.body ?? result.bodyText, t);
  return { ...data, subscriptionActiveUntil: liveSubscriptionActiveUntil ?? data.subscriptionActiveUntil };
};
...
export const CODEX_CONFIG: QuotaProviderData<CodexQuotaState, CodexQuotaData> = {
  ...
  buildSuccessState: (data) => ({
    ...
    rateLimitResetCreditsLoaded: data.rateLimitResetCreditsLoaded,
