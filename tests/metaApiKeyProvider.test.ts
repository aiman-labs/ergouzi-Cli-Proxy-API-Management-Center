...
test('updates only the selected duplicate Meta key and preserves unknown fields', async () => {
  const entries = [
    { 'api-key': 'duplicate', 'base-url': 'https://api.meta.ai/v1', 'future-field': 'first' },
    { 'api-key': 'duplicate', 'base-url': 'https://api.meta.ai/v1', 'future-field': 'second' },
  ];
  let written: unknown;
  apiClient.get = (async () => ({ 'meta-api-key': entries })) as typeof apiClient.get;
  apiClient.put = (async (_url: string, data: unknown) => { written = data; }) as typeof apiClient.put;
  await providersApi.updateMetaConfig('duplicate', 'https://api.meta.ai/v1', {
    apiKey: 'duplicate', baseUrl: 'https://api.meta.ai/v1', priority: 9,
  }, 1);
  expect(written).toEqual([entries[0], { ...entries[1], priority: 9 }]);
});
