...
describe('QuotaTimeline rendering', () => {
  ...
  test('renders an unexpired Codex reset credit as an expiry tick', () => {
    ...
    const expiry = new Date('2026-08-03T12:00:00Z');
    const localExpiry = `${String(expiry.getMonth() + 1).padStart(2, '0')}/${String(
      expiry.getDate()
    ).padStart(2, '0')} ${String(expiry.getHours()).padStart(2, '0')}:${String(
      expiry.getMinutes()
    ).padStart(2, '0')}`;
